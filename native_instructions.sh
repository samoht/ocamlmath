#!/bin/sh
#@(#)Prepare native compilation for the calculation modules for Ocaml.
...
grep ".*[.]cmxa" tmp_hints | sed "s?^?${BASENAME}?p" > tmp_std_lib
...
echo "ocamlopt.opt -linkall -o "$2" "${OPTIONS}" compilation_source.ml"
